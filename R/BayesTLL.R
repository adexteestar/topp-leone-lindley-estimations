#' @title Bayesian and Classical Estimates of Topp-leone distribution
...
sim_TLL<-function(n,r,alpha1,theta1, k,p,q,k1){
  ...
  for (i in 1:r){
    ...
    x=data<-quantile(w,alpha1, theta1)
    hat<-try(stats::optim(c(alpha1,theta1),pdf,x=data,control = list(maxit = 1000)),silent=F)
    alpha_mle<-hat$par[1]
    theta_mle<-hat$par[2]
